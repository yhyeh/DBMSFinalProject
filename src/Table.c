...
Table_t *new_Table(char *file_name) {
    ...
    table->capacity = INIT_TABLE_USER_SIZE;
    table->len = 0;
    table->users = (User_t*)malloc(
                            sizeof(User_t) * INIT_TABLE_USER_SIZE);
    table->cache_map = (unsigned char*)malloc(sizeof(char)*INIT_TABLE_USER_SIZE);
    memset(table->cache_map, 0, sizeof(char)*INIT_TABLE_USER_SIZE);
    table->fp = NULL;
